...
public class BscScanClient : IBlockchainExplorerClient
{
    private readonly IEtherscanApi _api;
    private readonly string? _apiKey;
    private readonly int _chainId;
    private readonly ILogger<BscScanClient> _logger;
    ...
    public BscScanClient(
        ...
        ILogger<BscScanClient> logger)
    {
        _api = api;
        _apiKey = configuration["BlockchainApis:BinanceSmartChain:ApiKey"];
        _chainId = configuration.GetValue<int>("BlockchainApis:BinanceSmartChain:ChainId", 56);
        _logger = logger;
        ...
        _retryPolicy = Policy
            ...
            .WaitAndRetryAsync(
                ...
                {
                    _logger.LogWarning(
                        "Retry {RetryCount} for BscScan API after {TotalSeconds}s due to: {Message}",
    ...
    public async Task<IEnumerable<BlockchainTransaction>> GetTransactionsAsync(string address, int limit = 100)
    {
        try
        {
            ...
            {
                _logger.LogWarning("BscScan API error for {Address}: {Error}", address, normalTxsResponse.Result.ErrorMessage);
            }
            ...
            {
                _logger.LogWarning("BscScan API token TX error for {Address}: {Error}", address, tokenTxsResponse.Result.ErrorMessage);
            }
        ...
        {
            _logger.LogError(ex, "Error fetching transactions from BscScan for address {Address}", address);
    ...
    private BlockchainTransaction MapToBlockchainTransaction(EtherscanTransaction tx)
    {
        ...
        return new BlockchainTransaction
        {
            ...
            TokenSymbol = string.IsNullOrEmpty(tx.TokenSymbol) ? "BNB" : tx.TokenSymbol,
