...
public class BscMonitorService : IChainMonitorService
{
    private readonly BscScanClient _apiClient;
    ...
    private readonly ILogger<BscMonitorService> _logger;
    private readonly int _maxTransactionsPerPoll;

    public string NetworkName => "Binance Smart Chain";
    public int NetworkId => 2; // BSC network ID from seed data

    public BscMonitorService(
        BscScanClient apiClient,
        ...
        ILogger<BscMonitorService> logger)
    {
    ...
    public async Task MonitorWalletsAsync()
    {
        try
        {
            _logger.LogInformation("Starting BSC wallet monitoring");
            ...
            {
                _logger.LogDebug("No active wallets to monitor for BSC");
                await UpdateMonitoringStatusAsync(true);
                return;
            }

            _logger.LogInformation("Monitoring {Count} BSC wallets", activeWallets.Count);
            ...
            await UpdateMonitoringStatusAsync(true);
            _logger.LogInformation("Completed BSC wallet monitoring");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error during BSC wallet monitoring");
    ...
    private async Task MonitorSingleWalletAsync(int walletId, string address)
    {
        try
        {
            ...
            {
                _logger.LogWarning("Rate limit exceeded for BSC, skipping wallet {Address}", address);
                return;
            }
            ...
            if (processedCount > 0)
            {
                _logger.LogInformation(
                    "Processed {Count} new transactions for BSC wallet {Address}",
        ...
        {
            _logger.LogError(ex, "Error monitoring BSC wallet {Address}", address);
        }
    }
    ...
    private async Task UpdateMonitoringStatusAsync(bool isHealthy, string? errorMessage = null)
    {
        ...
        {
            _logger.LogError(ex, "Error updating monitoring status for BSC");
        }
    }
}
